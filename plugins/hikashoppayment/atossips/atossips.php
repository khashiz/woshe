...
class plgHikashoppaymentAtossips extends hikashopPaymentPlugin {
	...
	var $pluginConfig = array(
		...
		'instalments'=> array('ATOS_INSTALLMENTS', 'input'),
		'period'=> array('délai entre les échéances (mois)', 'input','1'),
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id){
		...
		switch(@$this->payment_params->mode) {
			case 'simplified':
				$transactionRef = array("s10TransactionId" => $this->compute_transactionId());
		...
		if(@$method->payment_params->period<100 && @$method->payment_params->period>0 && $this->payment_params->instalments>=2 && $this->payment_params->instalments<=50 && ($this->payment_params->force_instalments==1 || @$_SESSION['hikashop_multiple_instalments']==1)){
			$vars0["paymentPattern"] = "INSTALMENT";
			$dates = array();
			$amounts = array();
			$tref = array();
			$instalment_amount = str_replace(array('.',','),'', round($vars0 / $this->payment_params->instalments, 2) * 100);
	...
	function onPaymentNotification(&$statuses){
		...
		switch( $repCode ) {
			...
			default:
				...
				$details = "Unknown response from Atos Sips " . $repCode."\n\r /Atos Sips authorisation Id :".$response["authorisationId"];
