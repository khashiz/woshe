...
class hikashopFieldItem {
	...
	function getFieldName(&$field, $requiredDisplay = false, $classname = '') {
		...
		$for = '';
		if($requiredDisplay && !empty($field->field_required))
			$required = '<strong class="formRequired"><span class="uk-text-danger">&ensp;*&ensp;</span></strong>';
		if(!empty($classname))
	...
	function trans($name) {
		...
		$app = JFactory::getApplication();
		$config = hikashop_config();
		if(hikashop_isClient('administrator') && !$config->get('translate_HTML_value_in_backend_for_fields', 0) && strcmp(JText::_($val), strip_tags(JText::_($val))) !== 0)
			$trans = $val;
		else
			$trans = JText::_($val);
...
class hikashopFieldAjaxfile extends hikashopFieldItem {
	...
	function check(&$field,&$value, $oldvalue) {
		...
		if(is_array($value)) {
			if(!empty($field->field_options['multiple'])) {
				...
				$return = $this->_checkOneFile($value, $oldvalue);
				if(!$return)
					return $return;
			}
	...
	function _checkOneFile(&$value, $oldValue) {
		$fileClass = hikashop_get('class.file');
		$path = $fileClass->getPath('file');
		$hash = '';
...
class hikashopFieldDropdown extends hikashopFieldItem {
	...
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null){
		...
		if(strpos($options, 'class="') === false) {
			$options .= ' class="hikashop_field_dropdown uk-select font uk-width-1-1"';
		} else {
			$options = str_replace('class="', 'class="hikashop_field_dropdown uk-select font uk-width-1-1', $options);
		}
		$options .= empty($field->field_options['attribute']) ? '' : ' '.$field->field_options['attribute'];
