...
class hikashopFilterTypeClass extends hikashopClass {
	...
	function displayInList($filter, $divName, $selected, $tab=""){
		...
		$html = '<span id="filter_values_container_'.$filter->filter_namekey.'_'.$divName.'" class="uk-display-block filter_values_container">'.$html.'</span>';
	...
	function getDeleteButton($filter, $divName, $val, $html, $br, $onlyButton=false, $id=''){
		...
		if($onlyButton){
			...
			return '<a href="#" data-container-div="hikashop_filter_form_'.$divName.'" onclick="'.$empty.$submitOnClick.'return false;" title="'.JText::_('REMOVE_THIS_SELECTION').'" class="uk-hidden"><i class="fa fa-times-circle"></i></a>';
		}
		$html=trim($html,' ,');
		$ids = array();
		if(is_array($val)){
	...
	function trans($name){
		...
		return '<h4 class="font uk-text-secondary f500 uk-h5 uk-margin-small-bottom hikashop_filter_title">'.$trans.'</h4>';
	}
...
class hikashopTextClass extends hikashopFilterTypeClass{
	...
	function display($filter, $divName, &$parent, $datas=''){
		...
		$html = '<input name="filter_'.$filter->filter_namekey.'"'.$attributes.' type="text" id="filter_text_'.$filter->filter_namekey.'" value="'.$name.'">';
		$html = '<span id="filter_values_container_'.$filter->filter_namekey.'_'.$divName.'" class="uk-display-block filter_values_container">'.$html.'</span>';
			...
			if($size){
				...
				foreach($filter->filter_value as $key => $value){
					...
					if($key==0){
						...
						$html.='ffff<span class="hikashop_filter_checkbox"><input '.$onClick.' '.$checked.' type="'.$type.'" name="filter_'.$filter->filter_namekey.''.$tab.'" value="::'.$value.'" id="filter_'.$filter->filter_id.'_'.$value.'"/><label class="filter_'.$filter->filter_namekey.''.$tab.'" for="filter_'.$filter->filter_id.'_'.$value.'">'.JText::sprintf('X_AND_INFERIOR',$formatVal).'</label>'.$deleteButton.'</span>'.$br;
					...
					if($key==$size-1){
						...
						$html.='55435<span class="hikashop_filter_checkbox"><input '.$onClick.' '.$checked.' type="'.$type.'" name="filter_'.$filter->filter_namekey.''.$tab.'" value="'.$value.'::" id="filter_'.$filter->filter_id.'_'.$value.'_plus"/><label class="filter_'.$filter->filter_namekey.''.$tab.'" for="filter_'.$filter->filter_id.'_'.$value.'_plus">'.JText::sprintf('X_AND_SUPERIOR', $formatVal ).'</label>'.$deleteButton.'</span>'.$br;
						parent::getDeleteButton($filter, $divName, '', $html, true);
					}
				}
			}
		...
		if($filter->filter_data=='characteristic'){
			...
			if(!empty($characteristic_values)){
				foreach($characteristic_values as $val){
					...
					$html.='<span class="hikashop_filter_checkbox"><input class="uk-checkbox" '.$onClick.' '.$checked.' type="'.$type.'" name="filter_'.$filter->filter_namekey.$tab.'" value="'.$val->characteristic_id.'" id="filter_'.$filter->filter_id.'_'.$val->characteristic_id.'_'.$divName.'"/><label class="font uk-text-small uk-text-secondary uk-margin-small-right filter_'.$filter->filter_namekey.'_'.$val->characteristic_id.'_'.$divName.'" for="filter_'.$filter->filter_id.'_'.$val->characteristic_id.'_'.$divName.'">'.$val->characteristic_value.'</label>'.$deleteButton.'</span>'.$br;
				}
			}
		}
		...
		if($filter->filter_data == 'custom_field' && empty($filter->filter_value)) {
			...
			if(!empty($field) && is_array($field->field_value)) {
				foreach($field->field_value as $key => $val) {
					...
					if($type!='hidden') $input = '<input class="uk-checkbox" '.$onClick.' '.$checked.' type="'.$type.'" name="filter_'.$filter->filter_namekey.$tab.'" value="'.htmlentities($val[0], ENT_COMPAT, 'UTF-8').'" id="field_'.$filter->filter_id.'_'.$key.'_'.$divName.'">';
					$html.='<span class="hikashop_filter_checkbox">'.$input.'<label class="font uk-text-small uk-text-secondary uk-margin-small-right filter_'.$filter->filter_id.'_'.$key.'_'.$divName.'"  for="field_'.$filter->filter_id.'_'.$key.'_'.$divName.'">'.JText::_(JText::_($val[1])).'</label>'.$deleteButton.'</span>'.$br;
				}
			}
		}
...
";
		...
		$html = '<div id="slider_'.$filter->filter_namekey.'_'.$divName.'" data-container-div="hikashop_filter_form_'.$divName.'">'.$html.'</div>';
		if(!empty($js))
			$html .= '<script>'.$js.'</script>';
		if(!empty($filter->filter_options['input'])) {
			$html = '<div class="uk-margin-small-bottom"><span class="slider_input_start_group uk-text-tiny uk-text-secondary font">
	...
	$after_input.
'</span>
<span class="slider_input_end_group uk-text-tiny uk-text-secondary font">
...
class hikashopInStockCheckboxClass extends hikashopFilterTypeClass{
	...
	function display($filter, $divName, &$parent, $datas='', $type='',$tab=''){
		...
		$html.= '<span class="hikashop_filter_checkbox"><input class="uk-checkbox" data-container-div="hikashop_filter_form_'.$divName.'"'.$onClick.' '.$checked.' type="checkbox" id="filter_'.$filter->filter_namekey.$tab.'_checkbox" name="filter_'.$filter->filter_namekey.$tab.'_checkbox" value="in_stock"><label for="filter_'.$filter->filter_namekey.$tab.'_checkbox" class="font uk-text-small uk-text-secondary uk-margin-small-right">'.$filter->filter_name.'</label><input id="filter_'.$filter->filter_namekey.$tab.'_hidden" type="hidden" name="filter_'.$filter->filter_namekey.$tab.'" value="'.$value.'"></span>';
		return $html;
	}
}
