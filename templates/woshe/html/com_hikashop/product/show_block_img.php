...
if(!$this->config->get('thumbnail')) {
	if(!empty ($this->element->images)) {
		echo '<img itemprop="image" src="' . $this->image->uploadFolder_url . $image->file_path . '" alt="' . $image->file_name . '" id="hikashop_main_image" />';
	}
} else {
	$style = '';
...
?>
		<div class="hikashop_product_main_image_thumb<?php echo $imgMode_class; ?>" id="hikashop_image_main_thumb_div<?php echo $variant_name;?>" >
<?php
	echo $prev_btn;
...
<?php
	if($this->image->override) {
		echo $this->image->display(@$image->file_path, true, @$image->file_name, 'id="hikashop_main_image'.$variant_name.'" itemprop="image" ','id="hikashop_main_image_link"', $width, $height);
	} else {
		if(empty($this->popup))
		...
		if(@$img->success) {
			...
			if (!empty ($this->element->images) && count($this->element->images) > 1)
				$attr .= ' class="uk-display-inline-block" onclick="return window.localPage.openImage(\'hikashop_main_image'.$variant_name.'\', \''.$variant_name.'\', event);"';
...
<!-- EO MAIN IMAGE -->
<!-- THUMBNAILS -->
	<div id="hikashop_small_image_div<?php echo $variant_name;?>" class="productThumbsWrapper">
        <div class="uk-child-width-1-5 uk-grid-small" data-uk-grid>
            ...
            if( !empty($this->element->images) && count($this->element->images) > 1) {
                ...
                foreach($this->element->images as $key => $image) {
                    ...
                    $attr = $thumbnail_class.' title="'.$this->escape(@$image->file_description).'" onmouseover="return window.localPage.changeImage('
                    ...
                    if(empty($variant_name)) {
                        echo '<div>'.$this->popup->image($html, $img->origin_url, $id, $attr, array('gallery' => 'hikashop_main_image')).'</div>';
                    } else {
