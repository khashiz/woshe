...
if($filter_type !== 3) {
	...
	if(!empty($html)){
		...
		if(!empty($htmlFilter) && $ctrl == 'category')
			echo $htmlFilter;
?>
	<div class="hikashop_products_listing uk-width-1-1 uk-width-expand@m">
<?php
...
?>
	...
	<div id="<?php echo $main_div_name.'_'.$category['category']->category_id; ?>" class="hikashop_products_listing bbbbb">
<?php
		if(hikaInput::get()->getVar('hikashop_front_end_main',0) && hikaInput::get()->getVar('task') == 'listing' && $this->params->get('show_compare')) {
			$css_button = $this->config->get('css_button', 'hikabtn');
			$css_button_compare = $this->config->get('css_button_compare', 'hikabtn-compare');
...
<?php
	...
	echo '<div class="listWrapper"><div data-uk-grid>'.$html.'</div></div>';
?>
	</div>
<?php
}
