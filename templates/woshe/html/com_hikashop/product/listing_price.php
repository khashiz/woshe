...
?>
	...
	if(empty($this->row->prices)) {
		...
		foreach($this->row->prices as $k => $price) {
			...
			$classes = array('font uk-text-secondary f500 hikashop_product_price hikashop_product_price '.$i);
			...
			if(!empty($this->row->discount)) {
				if(in_array($this->params->get('show_discount'), array(1, 4))) {
					echo '<span class="hikashop_product_discount font uk-text-muted uk-text-linethrough uk-text-small f500">'.JText::_('PRICE_DISCOUNT_START').'<span class="hikashop_product_discount_amount">';
				...
				if(in_array($this->params->get('show_discount'), array(2, 4))) {
					echo '<span class="uk-text-small uk-text-linethrough uk-display-block uk-text-muted hikashop_product_price_before_discount">'.JText::_('PRICE_DISCOUNT_START').'<span class="hikashop_product_price_before_discount_amount uk-text-tiny">';
