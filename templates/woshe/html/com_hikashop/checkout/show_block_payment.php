...
?>
<h5 class="uk-display-block font f600 uk-text-dark uk-margin-remove-top"><?php echo JText::_('HIKASHOP_PAYMENT_METHOD');?></h5>
<?php
	}
...
?>
	<fieldset class="hika_payment_field hikashop_checkout_payment_block">
		<select id="hikashop_payment_selector_<?php echo $this->step.'_'.$this->module_position; ?>"
				...
				class="frffrfrfrf hikashop_field_dropdown"
...
?>
	...
	<input class="uk-radio uk-margin-remove hikashop_checkout_payment_radio" type="radio" name="checkout[payment][id]" id="<?php echo $input_id; ?>" data-hk-checkout="<?php echo $this->escape(json_encode($input_data)); ?>" onchange="window.checkout.paymentSelected(this);" value="<?php echo $payment->payment_id;?>"<?php echo ($selected ? ' checked="checked"' : ''); ?>/>
                </div>
<?php } ?>
            <div class="uk-width-expand">
	<label for="<?php echo $input_id; ?>" class="uk-text-small font f600 uk-display-block uk-text-primary"><span class="uk-display-block hikashop_checkout_payment_name"><?php echo $payment->payment_name;?></span></label>
	<span class="hikashop_checkout_payment_cost uk-hidden"><?php echo $this->checkoutHelper->getDisplayPrice($payment, 'payment', $this->options); ?></span>
...
<?php if(!empty($payment->payment_description)) { ?>
	<div class="hikashop_checkout_payment_description uk-text-tiny font f600 uk-display-block uk-text-muted"><?php echo strip_tags($this->getDescription($payment)); ?></div>
<?php } ?>
    </div>
        <?php } ?>
<?php
