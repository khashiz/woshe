...
?>
	<h5 class="uk-display-block font f600 uk-text-dark uk-margin-remove-top"><?php echo JText::_('HIKASHOP_SHIPPING_METHOD');?></h5>
<?php
		}
...
?>
	<fieldset class="hika_shipping_field hikashop_checkout_shipping_block">
		<select id="hikashop_shipping_selector_<?php echo $this->step.'_'.$this->module_position; ?>"
				...
				class="fff  hikashop_field_dropdown"
...
?>
		...
		<input class="uk-radio uk-margin-remove hikashop_checkout_shipping_radio" type="radio" name="checkout[shipping][<?php echo $shipping_group_key; ?>][id]" id="<?php echo $input_id; ?>" data-hk-checkout="<?php echo $this->escape(json_encode($input_data)); ?>" onchange="window.checkout.shippingSelected(this);" value="<?php echo $shipping->shipping_id;?>"<?php echo ($selected ? ' checked="checked"' : ''); ?>/>
                    </div>
<?php
				}
?>
                <div class="uk-width-expand">
		<label for="<?php echo $input_id; ?>" class="uk-text-small font f600 uk-display-block uk-text-primary">
			<span class="uk-display-inline-block hikashop_checkout_shipping_name"><?php echo $shipping->shipping_name;?></span>
            <span class="uk-display-inline-block hikashop_checkout_shipping_cost"><?php echo '( '.$this->checkoutHelper->getDisplayPrice($shipping, 'shipping', $this->options).' )'; ?></span>
		</label>
<?php
...
?>
		<div class=" uk-text-tiny font f600 uk-display-block uk-text-muted hikashop_checkout_shipping_description"><?php
			echo strip_tags($this->getDescription($shipping));;
		?></div>
<?php
				} ?>
    </div>
