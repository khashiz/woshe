...
(function($, undefined) {
  ...
  function Datepicker() {
    ...
    this.dpDiv = bindHover($('<div id="' + this._mainDivId + '" class="regForm ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>'))
  }
  $.extend(Datepicker.prototype, {
    ...
    _generateHTML: function(inst) {
      ...
      var prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ? '<a class="uk-hidden uk-text-small uk-text-dark font uk-button uk-button-default uk-border-rounded uk-padding-remove uk-flex uk-flex-middle uk-flex-center uk-flex-first ui-datepicker-prev ui-corner-all" onclick="DP_jQuery_' + dpuuid + ".datepicker._adjustDate('#" + inst.id + "', -" + stepMonths + ", 'M');\" title=\"" + prevText + '"><span class="ui-icon ui-icon-circle-triangle-' + (isRTL ? "e" : "w") + '">' + prevText + "</span></a>" : (hideIfNoPrevNext ? "" : '<a style="direction:ltr" class="ui-datepicker-prev ui-corner-all ui-state-disabled" title="' + prevText + '"><span class="ui-icon ui-icon-circle-triangle-' + (isRTL ? "e" : "w") + '">' + prevText + "</span></a>"));
      ...
      var next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ? '<a class="uk-hidden uk-text-small uk-text-dark font uk-button uk-button-default uk-border-rounded uk-padding-remove uk-flex uk-flex-middle uk-flex-center uk-flex-last ui-datepicker-next ui-corner-all" onclick="DP_jQuery_' + dpuuid + ".datepicker._adjustDate('#" + inst.id + "', +" + stepMonths + ", 'M');\" title=\"" + nextText + '"><span class="ui-icon ui-icon-circle-triangle-' + (isRTL ? "w" : "e") + '">' + nextText + "</span></a>" : (hideIfNoPrevNext ? "" : '<a style="direction:ltr" class="ui-datepicker-next ui-corner-all ui-state-disabled" title="' + nextText + '"><span class="ui-icon ui-icon-circle-triangle-' + (isRTL ? "w" : "e") + '">' + nextText + "</span></a>"));
      ...
      for (var row = 0; row < numMonths[0]; row++) {
        ...
        for (var col = 0; col < numMonths[1]; col++) {
          ...
          calender += '<div class="uk-text-zero uk-flex ui-datepicker-header ui-widget-header ui-helper-clearfix' + cornerClass + '">' + (/all|left/.test(cornerClass) && row == 0 ? (isRTL ? next : prev) : "") + (/all|right/.test(cornerClass) && row == 0 ? (isRTL ? prev : next) : "") + this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate, row > 0 || col > 0, monthNames, monthNamesShort) + '</div><hr class="uk-margin-remove"><table class="ui-datepicker-calendar"><thead><tr>';
    ...
    _generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate, secondary, monthNames, monthNamesShort) {
      ...
      var html = '<div class="ui-datepicker-title uk-flex-1 uk-text-center uk-text-dark uk-text-small font uk-grid-small uk-child-width-auto uk-flex-center" data-uk-grid>';
      var monthHtml = "";
      if (secondary || !changeMonth) {
        monthHtml += '<div class="ui-datepicker-month">' + monthNames[drawMonth] + "</div>"
      } else {
        var inMinYear = (minDate && minDate.getFullYear() == drawYear);
        var inMaxYear = (maxDate && maxDate.getFullYear() == drawYear);
        monthHtml += '<div><select class="ui-datepicker-month uk-select uk-border-rounded" onchange="DP_jQuery_' + dpuuid + ".datepicker._selectMonthYear('#" + inst.id + "', this, 'M');\" onclick=\"DP_jQuery_" + dpuuid + ".datepicker._clickMonthYear('#" + inst.id + "');\">";
      ...
      if (!inst.yearshtml) {
        ...
        if (secondary || !changeYear) {
          html += '<div class="ui-datepicker-year">' + drawYear + "</div>"
        } else {
          ...
          inst.yearshtml += '<div><select class="ui-datepicker-year uk-select uk-border-rounded" onchange="DP_jQuery_' + dpuuid + ".datepicker._selectMonthYear('#" + inst.id + "', this, 'Y');\" onclick=\"DP_jQuery_" + dpuuid + ".datepicker._clickMonthYear('#" + inst.id + "');\">";
